...
class EmailVerificationHandler:
    def __init__(self,account=None):
        ...
        Args:
            account: 用於註冊的郵箱地址，與接收驗證碼的郵箱可能不同
        """
        config = Config()
        ...
        if not self.imap:
            # 如果不使用 IMAP，則使用臨時郵箱
            temp_mail = f"{self.username}{self.emailExtension}"
            logging.info(f"使用臨時郵箱接收驗證碼: {temp_mail}")
            self.receive_email = temp_mail
        else:
            # 使用 IMAP 設定的郵箱接收驗證碼
            self.receive_email = self.imap.get('imap_user', '')
            logging.info(f"使用 IMAP 郵箱接收驗證碼: {self.receive_email}")
    ...
    def _get_mail_code_by_imap(self, retry = 0):
        ...
        try:
            ...
            if search_by_date:
                ...
                # 使用接收郵件的地址而非註冊郵件的地址搜索
                search_email = self.receive_email if hasattr(self, 'receive_email') else self.account
                logging.info(f"使用郵箱地址搜索郵件: {search_email}")
                status, messages = mail.search(None, 'TO', f'"{search_email}"')
            if status != 'OK':
                return None
    ...
    def _get_latest_mail_code(self):
        ...
        if not mail_list_data.get("result"):
            logging.warning(f"查詢臨時郵箱未返回郵件結果: {self.receive_email}")
            return None, None

        # 获取最新邮件的ID
        first_id = mail_list_data.get("first_id")
        if not first_id:
            logging.warning(f"臨時郵箱中未找到郵件: {self.receive_email}")
            return None, None
        ...
        if not mail_detail_data.get("result"):
            logging.warning(f"獲取郵件內容失敗: {self.receive_email}")
            return None, None
        ...
        if code_match:
            code = code_match.group()
            logging.info(f"在郵件內容中找到驗證碼: {code}")
            return code, first_id
        
        logging.warning(f"在郵件內容中未找到驗證碼")
        return None, None

...
class Config:
    def __init__(self):
        ...
        # 如果 TEMP_MAIL_EXT 包含完整的電子郵件地址，直接使用它
        if "@" in self.temp_mail_ext:
            self.temp_mail = self.temp_mail_ext.split("@")[0]
            self.temp_mail_ext = "@" + self.temp_mail_ext.split("@")[1]
        # 否則，如果 TEMP_MAIL 包含 @，分割它
        elif "@" in self.temp_mail:
            parts = self.temp_mail.split("@")
            self.temp_mail = parts[0]
            self.temp_mail_ext = "@" + parts[1]
        # 如果都不包含 @，則確保 temp_mail_ext 以 @ 開頭
        elif not self.temp_mail_ext.startswith("@"):
            self.temp_mail_ext = "@" + self.temp_mail_ext
